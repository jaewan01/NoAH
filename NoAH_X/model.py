...
class NoAH_X:
    ...
    def __init__(self, core_attr, fringe_attr, core_affinity_matrix, fringe_affinity_matrix, seed_prob, edge_num):
        self.nc = core_attr.shape[0]
        self.nf = fringe_attr.shape[0]
        self.n = self.nc + self.nf
        self.c2a = torch.bernoulli(core_attr).to(torch.float)
        self.f2a = torch.bernoulli(fringe_attr).to(torch.float)
        self.m = edge_num
...
class NoAH_X_plus:
    ...
    def construct_hypergraph(self):
        ...
        for num_edge in tqdm.trange(self.m):
            ...
            cur_c2a = torch.bernoulli(self.c2a).to(torch.float)
            cur_core_probs = torch.ones(self.nc)
            for i in range(self.k):
                cur_core_probs *= self.Tc[i][(cur_c2a[core_seed, i].to(torch.int) + cur_c2a[:, i].to(torch.int))]
            cur_core_probs[core_seed] = 0
            ...
            if len(core_group) > 1:
                e2p = torch.mean(cur_c2a[core_group], dim=0)
            else:
                e2p = cur_c2a[core_group[0]]
            edge_attr = torch.bernoulli(e2p.expand(self.nf, -1)).to(torch.int)
            
            cur_f2a = torch.bernoulli(self.f2a).to(torch.float)
            attr_sum = (cur_f2a + edge_attr).to(torch.int)
