    ...
    if "sv" in fname:
        with open(dir_path + fname + ".txt", "r") as f:
            ...
            for li, line in enumerate(f.readlines()):
                ...
                X.append(li + 1)
            X = sorted(X)
            if dataname not in ["tags-ask-ubuntu", "tags-math-sx", "threads-ask-ubuntu", "threads-math-sx", "coauth-MAG-Geology-full", "coauth-MAG-History-full"]:
...
def powerlaw_test(test_type):
    # test_type in ["degree", "size", "pairdeg", "intersection"]
    fname = test_type

    dataset = ["email-Enron-full", "email-Eu-full", "contact-high-school", "contact-primary-school", "NDC-classes-full", "NDC-substances-full", "tags-math-sx", "tags-ask-ubuntu", "threads-math-sx" , "threads-ask-ubuntu" , "coauth-MAG-Geology-full", "coauth-MAG-History-full"] 
    #, "house-bills", "house-committees", "senate-bills", "senate-committees", "trivago-clicks", "walmart-trips"]
    ...
    for data_name in tqdm(dataset):
        ...
        for c in ['power_law', 'truncated_power_law', 'lognormal']: #, 'truncated_power_law', 'lognormal']:
            try:
