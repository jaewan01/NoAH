...
def prep_dataset_categorical(data_name):
    
    """
        Dataset Organization
        |__ dataset/"dataset_name"/                   
            |__ hyperedge.txt : Contains list of hyperedges. Each hyperedge is consists of nodes seperated by ",".
            |__ attribute_categorical.txt : Contains list of categorical node attribute vectors. i-th line indicates node i's attribute. Each attribute vector is consists of attributes seperated by ",". 
            |__ attribute_categorical_dims.txt : Contains list of dimensions for each categorical attribute. Each dimension is seperated by ",".
            
        Read hypergraph infomation from hyperedge.txt & attribute_cetegorical(_dims).txt, and return hyperedges, node attributes, number of nodes, number of hyperedges, and dimensions of node attributes.
    """
    ...
    with open(path + "attribute_categorical.txt", "r") as f:
        for line in f.readlines():
            cur_line = line.strip().split(",")
            attributes.append([int(i) for i in cur_line])
    
    attributes = torch.tensor(attributes)
    category_counts = []
    
    with open(path + "attribute_categorical_counts.txt", "r") as f:
        line = f.readline()
        cur_line = line.strip().split(",")
        category_counts = [int(i) for i in cur_line]
    
    attribute_dim = attributes.shape[1]
            
    return hyperedges, attributes, num_nodes, num_edges, category_counts, attribute_dim
